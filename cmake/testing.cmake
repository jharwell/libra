...
# Convention: Integration tests end with '-itest.c' or '-itest.cpp'
# Convention: Test harness bits end with '_test.c' or '_test.cpp'
file(GLOB LIBRA_c_utests ${${PROJECT_NAME}_TEST_PATH}/*-utest.c)
file(GLOB LIBRA_c_itests ${${PROJECT_NAME}_TEST_PATH}/*-itest.c)
file(GLOB LIBRA_cxx_utests ${${PROJECT_NAME}_TEST_PATH}/*-utest.cpp)
file(GLOB LIBRA_cxx_itests ${${PROJECT_NAME}_TEST_PATH}/*-itest.cpp)
file(GLOB LIBRA_c_test_harness ${${PROJECT_NAME}_TEST_PATH}/*_test.c ${${PROJECT_NAME}_TEST_PATH}/*.h ${${PROJECT_NAME}_TEST_PATH}/*.hpp)
file(GLOB LIBRA_cxx_test_harness ${${PROJECT_NAME}_TEST_PATH}/*_test.cpp  ${${PROJECT_NAME}_TEST_PATH}/*.hpp)
...
function(enable_single_utest t)
  ...
  # Add to global "build-and-test" target to build library+test
  # harness+unit tests and then run the unit tests
...
if(NOT TARGET ${PROJECT_NAME}-c-utest-harness AND
  "${LIBRA_HAVE_c_test_harness}" MATCHES "YES")
      add_library(${PROJECT_NAME}-c-utest-harness
        STATIC
        EXCLUDE_FROM_ALL
    ...
    # Have to link with the main project to propagate includes,
    # defines, etc.
    target_link_libraries(${PROJECT_NAME}-c-utest-harness
      ${PROJECT_NAME})
  endif()

  # Create C++ test harness
  if(NOT TARGET ${PROJECT_NAME}-cxx-utest-harness AND
      "${LIBRA_HAVE_cxx_test_harness}" MATCHES "YES")
    add_library(${PROJECT_NAME}-cxx-utest-harness
      STATIC
      EXCLUDE_FROM_ALL
    ...
    # on strange platforms requiring more exotic compilers.
    if ("${${PROJECT_NAME}_CHECK_LANGUAGE}" MATCHES "C")
      target_compile_options(${PROJECT_NAME}-cxx-utest-harness PUBLIC
        -Wno-old-style-cast
        -Wno-useless-cast)
    endif()
