...
function(libra_configure_exports TARGET PREFIX)
  ...
  if(NOT EXISTS "${PROJECT_SOURCE_DIR}/cmake/config.cmake.in")
    libra_message(FATAL_ERROR
                  "${PROJECT_SOURCE_DIR}/cmake/config.cmake.in does not exist")
  endif()
...
#
# Useful if your project has reusable functionality you want to expose to child
# projects.
function(libra_register_extra_configs_for_install TARGET FILES PREFIX)
  install(FILES ${FILES} DESTINATION "${PREFIX}/lib/cmake/${TARGET}")
endfunction()

# ##############################################################################
# Register the changelog to be installed at CMAKE_INSTALL_DOCDIR. If the name of
# the file is not 'changelog.gz' it is renamed to that.
#
# For use in configuring cpack to generate packages intended to be distributed
# as .deb packages.
#
function(libra_register_changelog_for_install TARGET FILE)
  ...
  add_custom_command(
    OUTPUT "${OUTPUT}"
    COMMAND gzip -cn9 "${CMAKE_CURRENT_SOURCE_DIR}/changelog" >
  ...
  add_custom_target(${TARGET}-changelog ALL
...
# ##############################################################################
# Register the copyright notice to be installed at CMAKE_INSTALL_DOCDIR. If the
# name of the file is not 'copyright' it is renamed to that.
#
# For use in configuring cpack to generate packages intended to be distributed
# as .deb packages.
#
