...
#
#   in_situ (default)
#     The original behaviour: CMAKE_MODULE_PATH is pointed at the cmake/
#     directory two levels above the tests/ tree.  Requires LIBRA_SOURCE_ROOT
#     to be set (injected by the test helper) or falls back to the relative
#     path that the original samples used.
#
#   add_subdirectory
#     LIBRA is pulled in via add_subdirectory().  LIBRA_SOURCE_ROOT must point
#     to the LIBRA repository root.  LIBRA's own root CMakeLists.txt is
#     expected to append cmake/ to CMAKE_MODULE_PATH; if it does not (i.e.
#     LIBRA has no root CMakeLists.txt) we append it ourselves.
#
#   installed_package
#     LIBRA has been installed to a prefix and is located via find_package().
#     CMAKE_PREFIX_PATH must already include the install prefix; the test
#     helper sets this via -DCMAKE_PREFIX_PATH=<prefix>.
#     After find_package() conan-style builddirs semantics don't apply here,
#     so we manually append the installed cmake/ dir to CMAKE_MODULE_PATH
#     using the libra_DIR hint that find_package() populates.
#
#   cpm
#     LIBRA is pulled in via CPM.cmake using a local SOURCE_DIR (no network
#     required).  LIBRA_CPM_CMAKE must point to CPM.cmake and LIBRA_SOURCE_ROOT
#     must point to the LIBRA repo root.  After CPMAddPackage() we append the
#     cmake/ subdir of the fetched source to CMAKE_MODULE_PATH.
#
#   conan
#     LIBRA has been installed into the Conan local cache via `conan create`
...
if(NOT DEFINED LIBRA_CONSUME_MODE OR LIBRA_CONSUME_MODE STREQUAL "in_situ")
  # ── in_situ ──────────────────────────────────────────────────────────────
  # Prefer the injected LIBRA_SOURCE_ROOT so the helper controls the path;
  # fall back to the historical relative path so samples still work when run
  # directly without going through the test harness.
...
elseif(LIBRA_CONSUME_MODE STREQUAL "add_subdirectory")
  ...
  if(NOT TARGET libra_module_path_set)
    ...
    if(EXISTS "${LIBRA_SOURCE_ROOT}/CMakeLists.txt")
      add_subdirectory("${LIBRA_SOURCE_ROOT}" "${CMAKE_BINARY_DIR}/_libra_src" EXCLUDE_FROM_ALL)
    endif()
...
elseif(LIBRA_CONSUME_MODE STREQUAL "installed_package")
  # ── installed_package ────────────────────────────────────────────────────
  # CMAKE_PREFIX_PATH is set by the test helper via -DCMAKE_PREFIX_PATH=<prefix>.
  # LIBRA is a cmake-module-only package so there is no compiled library to
  # link; find_package locates the config file and we use the resolved prefix
  # to set CMAKE_MODULE_PATH.
  find_package(libra REQUIRED CONFIG)
  # libra_DIR is set by find_package to the directory containing libra-config.cmake.
  # That file lives at <prefix>/cmake/libra-config.cmake, so one level up is
  # <prefix>/cmake — exactly what we need on CMAKE_MODULE_PATH.
...
elseif(LIBRA_CONSUME_MODE STREQUAL "cpm")
  ...
  if(NOT DEFINED LIBRA_SOURCE_ROOT)
    message(FATAL_ERROR
      "libra_consume: LIBRA_SOURCE_ROOT must be set for cpm mode")
  endif()
